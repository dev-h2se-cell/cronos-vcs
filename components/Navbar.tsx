...
import { ShoppingBag, Sun, Moon, Droplets, Shield } from 'lucide-react';

interface NavbarProps {
  cartCount: number;
  currentView: 'chronos' | 'infinity' | 'hydrolock' | 'shield';
  onNavigate: (view: 'chronos' | 'infinity' | 'hydrolock' | 'shield') => void;
}
...
export const Navbar: React.FC<NavbarProps> = ({ cartCount, currentView, onNavigate }) => {
  ...
  const themeBg = isDark ? 'bg-slate-950/90 border-slate-800' : isShield ? 'bg-[#FFFBF0]/90 border-amber-200' : 'bg-white/95 border-slate-200';
    ...
    <header className="fixed top-0 left-0 right-0 z-50 font-sans">
      ...
      <nav className={`transition-all duration-300 border-b ${scrolled ? `${themeBg} backdrop-blur-sm py-3 shadow-sm` : 'bg-transparent border-transparent py-5'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <div className="flex items-center bg-slate-200/50 p-1 rounded-full relative overflow-hidden backdrop-blur-sm w-[320px] md:w-[400px]">
             {/* Slider Background */}
             <div 
               className={`absolute top-1 bottom-1 w-[calc(25%-2px)] bg-white rounded-full shadow-md transition-all duration-500 ease-in-out z-0 
                 ${currentView === 'chronos' ? 'left-1' : 
                   currentView === 'hydrolock' ? 'left-[calc(25%)]' :
                   currentView === 'shield' ? 'left-[calc(50%)]' :
                   'left-[calc(75%)] bg-slate-800'
                 }`}
             ></div>

             <button 
               onClick={() => onNavigate('chronos')}
               className={`relative z-10 py-1.5 flex items-center justify-center gap-1 text-[10px] md:text-xs font-bold tracking-widest uppercase transition-colors duration-300 w-1/4 ${currentView === 'chronos' ? 'text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
             >
             ...
             <button 
               ...
               className={`relative z-10 py-1.5 flex items-center justify-center gap-1 text-[10px] md:text-xs font-bold tracking-widest uppercase transition-colors duration-300 w-1/4 ${currentView === 'hydrolock' ? 'text-cyan-700' : 'text-slate-500 hover:text-slate-700'}`}
             >
             ...
             <button 
               ...
               className={`relative z-10 py-1.5 flex items-center justify-center gap-1 text-[10px] md:text-xs font-bold tracking-widest uppercase transition-colors duration-300 w-1/4 ${currentView === 'shield' ? 'text-amber-700' : 'text-slate-500 hover:text-slate-700'}`}
             >
             ...
             <button 
               ...
               className={`relative z-10 py-1.5 flex items-center justify-center gap-1 text-[10px] md:text-xs font-bold tracking-widest uppercase transition-colors duration-300 w-1/4 ${currentView === 'infinity' ? 'text-white' : 'text-slate-500 hover:text-slate-700'}`}
             >
